...
# TODO rewrite to new pvn_dataset

class UnityPvn3d(Unity):
    ...
    #
    #     preprocessed_data_path = os.path.join(self.data_config.preprocessed_folder, "{:06}.bin".format(index))
    #     preprocessed_data = pickle.load(open(preprocessed_data_path, "rb"))
    #
    #     rgb = preprocessed_data['rgb'] / 255.  # normalize here
    #     pcld_info = preprocessed_data['pcld_info']
    #     crop_info = preprocessed_data['crop_info']
    #
    #     _, crop_down_factor = crop_info
    #
    #     crop_down_factor = 1  # todo fix hard code
    #
    #     crop_down_factor = np.array([crop_down_factor]).astype(np.int8)
    #
    #     pcld_xyz_rgb_nm, sampled_index = pcld_info
    #
    #     sampled_index = np.array(sampled_index).astype(np.int32)
    #
    ...
    #
    #         label_list, mask_label = label_info
    #
    #         ctr_targ_offst = np.array(ctr_targ_offst).astype(np.float32)
    #
    #         kpts_targ_offst = np.array(kpts_targ_offst).astype(np.float32)
    #
    #         mask_label = np.array(mask_label).astype(np.float32)
    #
    #         label_list = np.array(label_list).astype(np.int8)
    #
    #         if self.use_pvn_kp:
    #
